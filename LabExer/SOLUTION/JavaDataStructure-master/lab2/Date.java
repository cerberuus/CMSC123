...
class Date {
  ...
  public Date(int month, int day, int year) {
    if(!isValidDate(month, day, year)) {
      System.out.println("Fatal Error: Wrong valid date.");
    }
    this.month = month;
    this.day = day;
    this.year = year;
  ...
  public int difference(Date d) {
    ...
    if(larger.year > smaller.year) {
      diff = larger.dayInYear();
      for(int i = smaller.year; i < larger.year; i++) {
        if(isLeapYear(i)) {
          diff += 366;
        } else {
          diff += 365;
      ...
      diff -= smaller.dayInYear();
    } else {
      diff = larger.dayInYear() - smaller.dayInYear();
    } 
