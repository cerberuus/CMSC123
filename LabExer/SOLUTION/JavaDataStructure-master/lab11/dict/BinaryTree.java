...
public class BinaryTree implements Dictionary {
  ...
  private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
    ...
    int value = key.compareTo(node.entry.key());
    BinaryTreeNode returnNode;
    if (value < 0) {
      if (node.leftChild == null) {
        return null;
      } else {
        returnNode = findHelper(key, node.leftChild);
      }
    } else if(value > 0){
      if (node.rightChild == null) {
        return null;
      } else {
        returnNode = findHelper(key, node.rightChild);
      }
    } else {
      returnNode = node;
    }
    ...
    if (node == null) {
      ...
      if(node == root) {
        if(node.leftChild == null && node.rightChild == null) {
          ...
          BinaryTreeNode min_value = min(root.rightChild);
          if(min_value == root.rightChild){
            root.entry = min_value.entry;
            root.rightChild = null;
          } else {
            root.entry = min_value.entry;
            if(min_value.rightChild == null) {
              min_value.parent.leftChild = null;
            } else {
              min_value.rightChild.parent = min_value.parent;
              min_value.parent.leftChild = min_value.rightChild;
            }
          }
        }
        size--;
      ...
      if(node.leftChild == null && node.rightChild == null) {
        BinaryTreeNode parents = node.parent;
        ...
        BinaryTreeNode parents = node.parent;
        node.rightChild.parent = parents;
        if(parents.leftChild == node) {
          parents.leftChild = node.rightChild;
        } else {
          parents.rightChild = node.rightChild;
        }
      } else if(node.rightChild == null) {
        BinaryTreeNode parents = node.parent;
        node.leftChild.parent = parents;
        if(parents.leftChild == node) {
          parents.leftChild = node.leftChild;
        } else {
          parents.rightChild = node.leftChild;
        }
      } else {
        BinaryTreeNode minNode = min(node.rightChild);
        BinaryTreeNode parent_Min = minNode.parent;
        if(minNode == node.rightChild) {
          node.rightChild = null;
        } else {
          if(minNode.rightChild == null) {
            parent_Min.leftChild = null;
          } else {
            minNode.rightChild.parent = parent_Min;
            parent_Min.leftChild = minNode.rightChild;
          }
        }
