...
public class ListSorts {
  ...
  public static LinkedQueue makeQueueOfQueues(LinkedQueue q) {
    ...
    try {
      while(!q.isEmpty()) {
        LinkedQueue o = new LinkedQueue();
        o.enqueue(q.dequeue());
        link.enqueue(o);
      }
    } catch(QueueEmptyException e) {
      System.err.println ("Empty QueueEmptyException");
    }
    return link;
  }
  ...
  public static LinkedQueue mergeSortedQueues(LinkedQueue q1, LinkedQueue q2) {
    // Replace the following line with your solution.
    LinkedQueue sortOne = new LinkedQueue();
    try {
      while(!q1.isEmpty() && !q2.isEmpty()) {
        if(((Comparable)q1.front()).compareTo(((Comparable)q2.front())) < 0) {
          sortOne.enqueue(q1.dequeue());
        } else {
          sortOne.enqueue(q2.dequeue());
        }
      }
      if(!q1.isEmpty()) {
        sortOne.append(q1);
  ...
  public static void partition(LinkedQueue qIn, Comparable pivot, 
    ...
    try {
      while(!qIn.isEmpty()) {
        ...
        int value = pivot.compareTo((Comparable)item);
        if(value > 0) {
          qSmall.enqueue(item);
        } else if(value == 0) {
