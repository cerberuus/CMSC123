  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if(isValidNode()) {
      ...
      if(c <= 1) {
        ...
        SibTreeNode  cNode = (SibTreeNode)this.child(c - 1);
        if(!cNode.isValidNode()) {
          cNode = this.firstChild;
          while(cNode.nextSibling != null) {
            cNode = cNode.nextSibling;
          }
        }
        child.nextSibling = cNode.nextSibling;
        cNode.nextSibling = child;
      }
      myTree.size++;
    } else {
      throw new InvalidNodeException();
