...
public class Tree234 extends IntDictionary {
  ...
  public void insert(int key) {
    ...
    if (!find(key)) {
      do {
        if (node.keys == 3) {
          node = pushMiddle(node);
        }
        // Find the right leaf and add the key
  ...
  // push the middle key to the parent with key = 3
  private Tree234Node pushMiddle(Tree234Node node) {
    // if node is root
    if (node == root) {
      // create a new root and return it
      ...
      // newRoot.keys = updateKeys(newRoot);
      // newRoot.child1.keys = updateKeys(newRoot.child1);
      // newRoot.child2.keys = updateKeys(newRoot.child2);
      if (node.child1 != null) {
      ...
      if (node.parent.keys == 2) {
        if (node.key2 < node.parent.key1) {
          ...
          // node.parent.child1.keys = updateKeys(node.parent.child1);
          // node.parent.child2.keys = updateKeys(node.parent.child2);
          if (node.child3 != null) {
          ...
          // node.parent.child2.keys = updateKeys(node.parent.child2);
          // node.parent.child3.keys = updateKeys(node.parent.child3);
          if (node.child3 != null) {
        ...
        if (node.key2 < node.parent.key1) {
          ...
          // node.parent.child3.keys = updateKeys(node.parent.child3);
          // node.parent.child4.keys = updateKeys(node.parent.child4);
        }
      }
      node = node.parent;
    }
